...
vCenter credentials가 필요한 privilege를 가지고 있는지 검증합니다.
...
계산된 statistics와 time estimate를 조회합니다.
...
`estimatePer1TB` field는 1 TB data에 대한 migration time forecast를 제공합니다.
...
- **Disk size**: 큰 benchmark disk는 더 현실적인 결과를 제공하지만 prepare 시간이 길어집니다. 10 GB는 빠른 estimate에 적합한 default입니다.
